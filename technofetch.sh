...
detect_vm() {
    ...
    if [[ "$IS_VM" == "false" && -n "$dmi_lower" ]]; then
        ...
        elif echo "$dmi_lower" | grep -qi "proxmox"; then
            IS_VM="true"
            VM_HYPERVISOR="Proxmox (KVM)"
        fi
    fi

    # ── Method 5b: Proxmox-specific detection ──
    if [[ "$IS_VM" == "false" ]] || [[ "$VM_HYPERVISOR" == "KVM/QEMU" || "$VM_HYPERVISOR" == "KVM/QEMU (virtio)" ]]; then
        local is_proxmox_host="false"
        # Check for Proxmox host commands
        if has_cmd pveversion || has_cmd pvesh || has_cmd pvetest; then
            is_proxmox_host="true"
        fi
        # Check /etc/os-release for proxmox
        if [[ -r /etc/os-release ]]; then
            if grep -qi "proxmox" /etc/os-release 2>/dev/null; then
                is_proxmox_host="true"
            fi
        fi
        # Check Proxmox VE directory
        if [[ -d /etc/pve ]] || [[ -f /etc/pve/.vmlist ]]; then
            is_proxmox_host="true"
        fi
        # Check for Proxmox kernel
        if uname -r 2>/dev/null | grep -qi "pve"; then
            is_proxmox_host="true"
        fi
        if [[ "$is_proxmox_host" == "true" ]]; then
            IS_VM="false"  # Proxmox host is NOT a VM
            VM_HYPERVISOR="Proxmox VE"
            # Get Proxmox version
            if has_cmd pveversion; then
                local pve_ver
                pve_ver=$(pveversion 2>/dev/null | grep -oP 'pve-manager/\K[^/]+' || echo "")
                [[ -n "$pve_ver" ]] && VM_PRODUCT="PVE $pve_ver"
            fi
        fi
    fi
